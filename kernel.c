...
uint32_t read_rtc()
{
    uint32_t rtc_val = READ_CSR(time);
    return rtc_val;
}


//write to stimecmp
void write_to_stimecmp(uint64_t x)
{
    WRITE_CSR(0x14d, x);
}
...
void handle_exception_trap(struct trap_frame *f){
...
//timer interrupt handler without yield
__attribute__((naked))
__attribute__((aligned(4)))
